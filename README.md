[README.md](https://github.com/user-attachments/files/26760249/README.md)
# LUNEA
...
```
✦ EIGENPHASE (10 min)    KI gesperrt. Schüler:innen denken zuerst selbst.
◈ KI-PHASE   (10 min)    KI freigegeben. Begrenzte Prompts. Pädagogische Begleitung.
◉ FOKUSPHASE (10 min)    KI gesperrt. Eigenständige Weiterarbeit mit dem Gelernten.
◇ REFLEXION  (10 min)    Kein KI-Zugang. Transfer, Vergleich, Gruppenanalyse.
```
...
- Sie sieht alle Schülerprompts und kann Top-5-Analyse starten
- Sie entscheidet, wann Gruppenvergleich gezeigt wird
...
Jeder Prompt wird automatisch bewertet nach:
- **Präzision** – wie klar und spezifisch ist die Frage?
...
4. **Nächster besserer Prompt** – ein konkretes Beispiel, wie es stärker gegangen wäre
5. **Nächster Lernschritt** – was genau ist die nächste sinnvolle Handlung?

Kein generisches Lob. Keine Noten. Direktes, substanzielles Feedback.

### Top-5-Prompt-Analyse für Lehrkräfte

Die Lehrkraft kann jederzeit eine Analyse aller Schülerprompts starten. Das Ergebnis:
- Die 5 stärksten Prompts mit Begründung, warum sie stark sind
...
Die KI vergleicht dann alle Transfer-Sätze der Gruppe und zeigt:
- Was alle / die meisten gemeinsam verstanden haben
...
**Jahrgangsstufen 5–13** mit Themen orientiert am Kerncurriculum Niedersachsen.

Eigene Themen sind jederzeit zusätzlich eingebbar – unabhängig von der vorhandenen Struktur.
...
| Route | Funktion |
|---|---|
| `/api/chat` | KI-Chat mit pädagogischer Persona |
| `/api/prior-knowledge` | Themenspezifische Vorwissensfragen |
| `/api/rate-prompt` | Prompt-Bewertung in 3 Dimensionen |
| `/api/analyze` | Top-5-Gruppenanalyse + Schülerfeedback |
| `/api/group-compare` | Transferantworten der Gruppe vergleichen |
...
✦  EIGENPHASE
   ...
   notieren eigene Gedanken und beantworten Vorwissensfragen.
   Erst danach wird die KI freigeschaltet.

◈  KI-PHASE
   KI freigegeben mit Prompt-Limit.
   Jeder Prompt wird automatisch in 3 Dimensionen bewertet.
   Persönliches Feedback in 5 Dimensionen auf Abruf.
   Bild-Upload möglich.

◉  FOKUSPHASE
   KI erneut gesperrt. Schüler:innen arbeiten eigenständig weiter
   mit dem, was sie gelernt und erarbeitet haben.

◇  REFLEXION
   5 Reflexionsfragen. Transfer-Satz. Gruppenvergleich.
   KI nicht verfügbar.
```
